...
std::string utf8togb2312(const std::string& utf8str)
{
    ...
    try
    {
        wTemp = cutf8.from_bytes(utf8str);
    }
...
std::string read_whole_file_(const char* filename)
{
#if false
...
#else
    /// here's a way to do it that allocates all the memory up front
...
std::string read_whole_file(const char* filename, std::string* errMsg)
{
    try
    {
        return read_whole_file_(filename);
    }
    catch (const std::exception& e)
    {
        if (nullptr != errMsg)
        {
            *errMsg = std::string("read file exception: ") + filename + ", " +
                      e.what();
        }
    }
    return "";
...
void assign(char* r, int32_t v, int n)
{
	...
	size_t siz = std::min(size_t(n - 1), str.length());
	strncpy(r, str.c_str(), siz);
	r[siz] = 0;
#else
    char szTmp[20] = {0};
    sprintf(szTmp, "%d", v);
...
void assign(char* r, std::string& v, int n) { assign(r, v.c_str(), n); }
...
bool create_recursion_dir(const std::string& dirPath)
{
    ...
    while (pos != std::string::npos)
    {
        ...
        if (cur.length() > 0 && !is_directory_exists(cur))
        {
            bool ret = false;
#ifdef WIN32
...
int make_directory_tree(const char* pPath)
{
    ...
    str_path(path, p, n);
    path = p;
    if (is_directory_exists(path)) return 0;
...
csv_mapping(const std::string& title, const std::string& value,
            const std::string& dummy)
{
    ...
    cleanvalue = replace_whole(cleanvalue, " ", "");
    cleanvalue = replace_whole(cleanvalue, ",,", "," + dummy + ",");
    auto titletokens = split(title, ',');
    auto valuetokens = split(cleanvalue, ',');
    for (int i = 0; i < std::min(titletokens.size(), valuetokens.size()); ++i)
    {
        maps->insert(std::make_pair(titletokens[i], valuetokens[i]));
    }
    return maps;
}
