...
public:
    ...
    virtual int reload(const std::string& jsonContent = "") { return F_FAIL; };

    virtual StrategyInstId instanceId() const { return context_ == nullptr ? "" : context_->uuid(); };

    ContextPtr context() const { return context_; }

    virtual bool updateParameters(const std::string& jsonContent) { return true; }

    /// make a public data feed method.
    virtual void onData(int32_t dataType, const char* data, size_t datasiz) PURE;

    /// JSON请求的回复
    virtual void onJsonTrading(const ORJsonTradingRspFieldPtr& jsonTradingRsp){};
    ...
    /// 事务执行过程中的状态快照。
    virtual void onExecSnapshot(const trade::TransactionPtr& transaction, OROrderFieldPtr order){};

    /// 事务执行过程中的成交快照。
    virtual void onExecSnapshot(const trade::TransactionPtr& transaction, ORTradeFieldPtr trade){};
    ...
    /// 线程都不一样。
    virtual or_price_f updateOrderPrice(const trade::OrderPackInfoPtr& orderPack, const trade::TransactionPtr& transaction,
                                        bool clearCounter)
    {
    ...
    /// 默认不执行任何操作，以支持已有策略。
    virtual void onUnload(std::string& reason) { is_running_ = 1; };
