...
using IterateOrderPackCb = std::function<bool(trade::OrderPackInfoPtr&)>;
...
public:
    ...
    virtual uint32_t size() const PURE;

    /// 得到委托列表的当前镜像状态。线程安全。
    virtual OROrderList getOrdersSnapshot(or_execstatus_i statusMixed) const PURE;
    ...
    virtual bool iterate(IterateOrderPackCb callback) PURE;
};
