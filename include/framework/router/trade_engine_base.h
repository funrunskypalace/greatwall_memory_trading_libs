...
public:
    ...
    int start(RunMode runMode, const std::string& appId, bool flatInFlow);
    bool canStop() const;
    int stop();
...
private:
    virtual void onPositionChanged(or_channelid_i channelId, ORAccountPositionField* pRspPosition,
                                   ORRspInfoField* pRspInfo) override;
    virtual void onAccountAssetChanged(or_channelid_i channelId, ORAccountAssetField* pRspPosition,
                                       ORRspInfoField* pRspInfo) override;
...
protected:
    ...
    virtual void notifyTradeEngineStatus(const std::string& statusCode, const std::string& statusMsg){};
