...
#include "orchid/buffer/buffer.h"
#include "orchid/common/common.h"
#include "orchid/grpc/status.h"
#include "orchid/http/header_map.h"
#include "orchid/tracing/http_tracer.h"

#include "orchid/common/assert.h"
#include "orchid/protobuf/protobuf.h"
...
public:
     ...
     * @param end_stream close the stream locally. No further methods may be
     * invoked on the stream object, but callbacks may still be received until
     * the stream is closed remotely.
     */
    virtual void sendMessageRaw(Buffer::InstancePtr&& request,
    ...
    /**
     * Close the stream locally and remotely (as needed). No further methods may
     * be invoked on the stream object and no further callbacks will be invoked.
     */
...
public:
    ...
    /**
     * Called when the async gRPC request succeeds. No further callbacks will be
     * invoked.
     * @param response the gRPC response bytes.
     * @param span a tracing span to fill with extra tags.
     */
    virtual void onSuccessRaw(Buffer::InstancePtr&& response,
     ...
     * @param span a tracing span to fill with extra tags.
     */
    virtual void onFailure(Status::GrpcStatus status,
 ...
 * Note the gRPC stream is full-duplex, even if the local to remote stream has
 * been ended by AsyncStream.close(), AsyncStreamCallbacks can continue to
 * receive events until the remote to local stream is closed (onRemoteClose),
 * and vice versa. Once the stream is closed remotely, no further callbacks will
 * be invoked.
 */
...
public:
     ...
     * onReceiveTrailingMetadata() with the trailing metadata.
     * @param metadata initial metadata reference.
     */
   ...
   * @return bool which is true if the message well formed and false otherwise
   which will cause the stream to shutdown with an INTERNAL error.
   */
     ...
     * stream is considered remotely closed after this invocation and no further
     * callbacks will be invoked. In addition, no further stream operations are
     * permitted.
     * @param status the gRPC status.
     * @param message the gRPC status message or empty string if not present.
     */
    virtual void onRemoteClose(Status::GrpcStatus status,
                               const std::string& message) PURE;
};

/**
 * Supports sending gRPC requests and receiving responses asynchronously. This
 * can be used to implement either plain gRPC or streaming gRPC calls.
 */
...
public:
     ...
     * @param service_full_name full name of the service (i.e.
     ...
     * @return a request handle or nullptr if no request could be started. NOTE:
     * In this case onFailure() has already been called inline. The client owns
     * the request and the handle should just be used to cancel.
     */
    virtual AsyncRequest*
    sendRaw(absl::string_view service_full_name, absl::string_view method_name,
     ...
     * TODO(mattklein123): Determine if tracing should be added to streaming
     * requests.
     * @param service_full_name full name of the service (i.e.
     * service_method.service()->full_name()).
     * @param method_name name of the method (i.e. service_method.name()).
     * @param callbacks the callbacks to be notified of stream status.
     * @return a stream handle or nullptr if no stream could be started. NOTE:
     * In this case onRemoteClose() has already been called inline. The client
     * owns the stream and the handle can be used to send more messages or
     * finish the stream. It is expected that closeStream() is invoked by the
     * caller to notify the client that the stream resources may be reclaimed.
     */
