...
#include "orchid/common/utility.h"
#include "orchid/common/assert.h"
#include "orchid/common/interval_set.h"
#include "orchid/common/non_copyable.h"
...
uint64_t generate_xxh64(const void* data, size_t size, uint64_t seed);

/**
 * Utility class for formatting dates given an absl::FormatTime style format
 * string.
 */
class DateFormatter
{
public:
     ...
     * @return std::string representing the GMT/UTC time of a TimeSource based
...
private:
    ...
    std::string
    fromTimeAndPrepareSpecifierOffsets(time_t time,
    ...
    struct Specifier
    {
        ...
        Specifier(const size_t position, const size_t width,
        ...
        // To build a second-specifier (%s), the number of characters to be
        // replaced is always 2.
        ...
        // The width of a specifier, e.g. given %3f, the width is 3. If %f is
        // set as the specifier, the width value should be 9 (the number of
        // nanosecond digits).
        const size_t width_;

        // The string before the current specifier's position and after the
        // previous found specifier. A segment may include absl::FormatTime
        // accepted specifiers. E.g. given
        // "%3f-this-i%s-a-segment-%4f", the current specifier is "%4f" and the
        // segment is
        // "-this-i%s-a-segment-".
        const std::string segment_;

        // As an indication that this specifier is a %s (expect to be replaced
        // by seconds since the epoch).
        const bool second_;
    };
    ...
    // This is the format string as supplied in configuration, e.g. "foo %3f
    // bar".
...
/**
 * Class used for creating non-copying std::istream's. See
 * InputConstMemoryStream below.
 */
...
/**
 * std::istream class similar to std::istringstream, except that it provides a
 * view into a region of constant memory. It can be more efficient than
 * std::istringstream because it doesn't copy the provided string.
 *
...
public:
    /**
     * Callable struct that returns the result of string comparison ignoring
     * case.
    ...
    /**
     * Callable struct that returns the hash representation of a
     * case-insensitive string_view input.
     ...
     * @return pointer to the remainder of 'str' if successful, nullptr
     * otherwise.
     */
     ...
     * @param out_len supplies the length of the output buffer. Must be >=
     * MIN_ITOA_OUT_LEN.
     ...
     * @param multi-delimiter supplies chars used to split the
     * delimiter-separated string view.
     * @param token supplies the lookup string view.
     * @param trim_whitespace remove leading and trailing whitespaces from each
     * of the split string views; default = true.
     ...
     */
    static bool findToken(absl::string_view source,
                          absl::string_view delimiters, absl::string_view token,
     ...
     * @param trim_whitespace remove leading and trailing whitespaces from each
     * of the split string views; default = true.
     * @return true if found a string that is semantically the same and false
     * otherwise.
     *
     * E.g.,
     *
     ...
     */
    static bool caseFindToken(absl::string_view source,
                              absl::string_view delimiters,
                              absl::string_view key_token,
                              bool trim_whitespace = true);

    /**
     * Compare one string view with another string view ignoring case
     * sensitivity.
     * @param lhs supplies the first string view.
     * @param rhs supplies the second string view.
     * @return true if strings are semantically the same and false otherwise.
    ...
    /**
     * Crop characters from a string view starting at the first character of the
     * matched delimiter string view until the end of the source string view.
     * @param source supplies the string view to be processed.
     * @param delimiter supplies the string view that delimits the starting
     * point for deletion.
     ...
     * cropRight("foo ; ; ; ; ; ; ", ";") == "foo "
     */
    static absl::string_view cropRight(absl::string_view source,
                                       absl::string_view delimiters);

    /**
     * Crop characters from a string view starting at the first character of the
     * matched delimiter string view until the beginning of the source string
     * view.
     * @param source supplies the string view to be processed.
     * @param delimiter supplies the string view that delimits the starting
     * point for deletion.
     ...
     * cropLeft("foo ; ; ; ; ; ", ";") == " ; ; ; ; "
     */
    static absl::string_view cropLeft(absl::string_view source,
                                      absl::string_view delimiters);
     ...
     * @param multi-delimiter supplies chars used to split the
     * delimiter-separated string view.
     * @param keep_empty_string result contains empty strings if the string
     * starts or ends with 'split', or if instances of 'split' are adjacent;
     * default = false.
     * @return vector containing views of the split strings
     */
    static std::vector<absl::string_view>
    splitToken(absl::string_view source, absl::string_view delimiters,
               bool keep_empty_string = false);

    /**
     * Remove tokens from a delimiter-separated string view. The tokens are
     * trimmed before they are compared ignoring case with the elements of
     * 'tokens_to_remove'. The output is built from the trimmed tokens
     * preserving case.
     * @param source supplies the delimiter-separated string view.
     * @param multi-delimiters supplies chars used to split the
     * delimiter-separated string view.
     * @param tokens_to_remove supplies a set of tokens which should not appear
     * in the result.
     * @param joiner contains a string used between tokens in the result.
     * @return string of the remaining joined tokens.
     */
    static std::string removeTokens(absl::string_view source,
                                    absl::string_view delimiters,
     ...
     */
    static std::string subspan(absl::string_view source, size_t start,
                               size_t end);
    ...
    nonEmptyStringOrDefault(const std::string& s,
     ...
     */
    static std::string
    removeCharacters(const absl::string_view& str,
...
public:
    ...
    pickCluster(const std::vector<WeightedClusterEntry>& weighted_clusters,
                const uint64_t total_cluster_weight,
                const uint64_t random_value, const bool ignore_overflow)
    {
        ...
        // [0, cluster1_weight), [cluster1_weight,
        ...
        for (const WeightedClusterEntry& cluster : weighted_clusters)
        {
            ...
            if (!ignore_overflow)
            {
                // end > total_cluster_weight: This case can only occur with
                // Runtimes, when the user specifies invalid weights such that
...
/**
 * Maintains sets of numeric intervals. As new intervals are added, existing
 * ones in the set are combined so that no overlapping intervals remain in the
 * representation.
 *
...
public:
    ...
    void insert(Value left, Value right) override
    {
        ...
        // There 3 cases where we'll decide the [left, right) is disjoint with
        // the current contents, and just need to insert. But we'll structure
        // the code to search for where existing interval(s) needs to be merged,
        // and fall back to the disjoint insertion case.
        if (!intervals_.empty())
        {
            ...
            if (left_pos != intervals_.end() && (right >= left_pos->first))
            {
                ...
                if (right_pos != intervals_.begin())
                {
                    ...
                    {
                        // Both bounds overlap, with one or more existing
                        // intervals.
...
/**
 * Hashing functor for use with unordered_map and unordered_set with string_view
 * as a key.
 */
struct StringViewHash
{
    ...
    {
        return generate_xxh64(k.data(), k.size(), 0);
    }
};

/**
 * Hashing functor for use with enum class types.
 * This is needed for GCC 5.X; newer versions of GCC, as well as clang7, provide
 * native hashing specializations.
 */
...
/**
 * A trie used for faster lookup with lookup time at most equal to the size of
 * the key.
 */
template <class Value> struct TrieLookupTable
{
     ...
     * @param overwrite_existing will overwrite the value when the value for a
    ...
    /**
     * Finds the entry associated with the longest prefix. Complexity is
     * O(min(longest key prefix, key length))
     * @param key the key used to find.
...
// consider this case:
//   struct MyStruct : public InlineStorage { uint64_t a_; uint16_t b_; uint8_t
//   data_[]; };
// When compiled with a typical compiler on a 64-bit machine:
//   sizeof(MyStruct) == 16, because the compiler will round up from 10 for
//   uint64_t alignment.
// So:
//   calling new (6) MyStruct() causes an allocation of 16+6=22, rounded up to
//   24 bytes.
// But data_ doesn't need 8-byte alignment, so it will wind up adjacent to the
// uint16_t.
//   ((char*) my_struct.data) - ((char*) &my_struct) == 10
// If we had instead declared data_[6], then the whole allocation would have fit
// in 16 bytes. Instead:
//   - the starting address of data will not be 8-byte aligned. This is not
//   required
//     by the C++ standard for a uint8_t, but may be suboptimal on some
//     processors.
//   - the 6 bytes of data will be at byte offsets 10 to 15, and bytes 16 to 23
//   will be
//     unused. This may be surprising to some users, and suboptimal in resource
//     usage.
// One possible tweak is to declare data_ as a uint64_t[], or to use an
// `alignas` declaration. As always, micro-optimizations should be informed by
// microbenchmarks, showing the benefit.
class InlineStorage : public NonCopyable
{
public:
    // Custom delete operator to keep C++14 from using the global operator
    // delete(void*, size_t), which would result in the compiler error:
...
protected:
    /**
     * @param object_size the size of the base object; supplied automatically by
     * the compiler.
     * @param data_size the amount of variable-size storage to be added, in
     * bytes.
...
public:
    ...
    {
        return InlineStringPtr(new (str.size())
