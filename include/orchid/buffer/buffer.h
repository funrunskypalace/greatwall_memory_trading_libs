...
#include "orchid/api/os_sys_calls.h"
...
/**
 * A wrapper class to facilitate passing in externally owned data to a buffer
 * via addBufferFragment. When the buffer no longer needs the data passed in
 * through a fragment, it calls done() on it.
 */
...
public:
    ...
    /**
     * Add externally owned data into the buffer. No copying is done. fragment
     * is not owned. When the fragment->data() is no longer needed,
     * fragment->done() is called.
    ...
    /**
     * Commit a set of slices originally obtained from reserve(). The number of
     * slices should match the number obtained from reserve(). The size of each
     * slice can also be altered. Commit must occur following a reserve()
     * without any mutating operations in between other than to the iovecs len_
     * fields.
     ...
     * @return the actual number of slices needed, which may be greater than
     * out_size. Passing nullptr for out and 0 for out_size will just return the
     * size of the array needed to capture all of the slice data.
     * TODO(mattklein123): WARNING: The underlying implementation of this
     * function currently uses libevent's evbuffer. It has the infuriating
     * property where calling getRawSlices(nullptr, 0) will return the slices
     * that include all of the buffer data, but not any empty slices at the end.
     * However, calling getRawSlices(iovec, SOME_CONST), WILL return potentially
     * empty slices beyond the end of the buffer. Code that is trying to avoid
     * stack overflow by limiting the number of returned slices needs to deal
     * with this. When we get rid of evbuffer we can rework all of this.
     */
    virtual uint64_t getRawSlices(RawSlice* out, uint64_t out_size) const PURE;

    /**
     * @return uint64_t the total length of the buffer (not necessarily
     * contiguous in memory).
     */
    virtual uint64_t length() const PURE;

    /**
     * @return a pointer to the first byte of data that has been linearized out
     * to size bytes.
     */
    ...
    /**
     * Move a portion of a buffer into this buffer. As little copying is done as
     * possible.
     * @param rhs supplies the buffer to move.
     ...
     */
    virtual uint64_t reserve(uint64_t length, RawSlice* iovecs,
                             uint64_t num_iovecs) PURE;
     ...
     * @param Endianness specifies the byte order to use when decoding the
     * integer.
     * @details Size parameter: Some protocols have integer fields whose size in
     * bytes won't match the size in bytes of C++'s integer types. Take a 3-byte
     * integer field for example, which we want to represent as a 32-bit (4
     * bytes) integer. One option to deal with that situation is to read 4 bytes
     * from the buffer and ignore 1. There are a few problems with that
     * solution, though.
     *   * The first problem is buffer underflow: there may not be more than
     * Size bytes available (say, last field in the payload), so that's an edge
     * case to take into consideration.
     *   * The second problem is draining the buffer after reading. With the
     * above solution we cannot read and discard in one go. We'd need to peek 4
     * bytes, ignore 1 and then drain 3. That not only looks hacky since the
     * sizes don't match, but also produces less terse code and requires the
     * caller to propagate that logic to all call sites. Things complicate even
     * further when endianness is taken into consideration: should the most or
     * least-significant bytes be padded? Dealing with this situation requires a
     * high level of care and attention to detail. Properly calculating which
     * bytes to discard and how to displace the data is not only error prone,
     * but also shifts to the caller a burden that could be solved in a much
     * more generic, transparent and well tested manner.
     *   * The last problem in the list is sign extension, which should be
     * properly handled when reading signed types with negative values. To make
     * matters easier, the optional Size parameter can be specified in those
     * situations where there's a need to read less bytes than a C++'s integer
     * size in bytes. For the most common case when one needs to read exactly as
     * many bytes as the size of C++'s integer, this parameter can simply be
     * omitted and it will be automatically deduced from the size of the type T
     */
    template <typename T, ByteOrder Endianness = ByteOrder::Host,
              size_t Size = sizeof(T)>
    T peekInt(uint64_t start = 0)
    {
        static_assert(Size <= sizeof(T),
                      "requested size is bigger than integer being read");

        if (length() < start + Size)
        {
            throw OrchidException("buffer underflow");
        }

        constexpr const auto displacement =
            Endianness == ByteOrder::BigEndian ? sizeof(T) - Size : 0;

        auto result = static_cast<T>(0);
        constexpr const auto all_bits_enabled =
        ...
        // If Size == sizeof(T), we need to make sure we don't generate an
        // invalid left shift (e.g. int32 << 32), even though we know that that
        // branch of the conditional will. not be taken. Size % sizeof(T) gives
        // us the correct left shift when Size < sizeof(T), and generates a left
        // shift of 0 bits when Size == sizeof(T)
        ...
        return fromEndianness<Endianness>(static_cast<T>(result)) |
               sign_extension_bits;
    }
     ...
     * @param value supplies the byte to copy into the buffer.
     */
    template <typename T> void writeByte(T value)
    {
     ...
     * @param Endianness specifies the byte order to use when encoding the
     * integer.
     */
    template <ByteOrder Endianness = ByteOrder::Host, typename T,
              size_t Size = sizeof(T)>
    void writeInt(T value)
    {
        static_assert(Size <= sizeof(T),
                      "requested size is bigger than integer being written");

        const auto data = toEndianness<Endianness>(value);
        constexpr const auto displacement =
            Endianness == ByteOrder::BigEndian ? sizeof(T) - Size : 0;
        add(reinterpret_cast<const char*>(std::addressof(data)) + displacement,
            Size);
    }
...
/**
 * A factory for creating buffers which call callbacks when reaching high and
 * low watermarks.
 */
...
public:
     ...
     * @param below_low_watermark supplies a function to call if the buffer goes
     * under a configured low watermark.
     * @param above_high_watermark supplies a function to call if the buffer
     * goes over a configured high watermark.
