...
public:
    ...
    /**
     * Write the buffer to the file. The file must be explicitly opened before
     * writing.
     *
...
{
public:
    static std::unique_ptr<Instance> createFileSystem();
    ...
    /**
     * @return bool whether a directory exists on disk and can be opened for
     * read.
     */
    virtual bool directoryExists(const std::string& path) PURE;

    /**
     * @return ssize_t the size in bytes of the specified file, or -1 if the
     * file size cannot be determined for any reason, including without
     * limitation the non-existence of the file.
     */
    virtual ssize_t fileSize(const std::string& path) PURE;

    /**
     * @return full file content as a string.
     * @throw OrchidException if the file cannot be read.
    ...
    /**
     * Determine if the path is on a list of paths orchid will refuse to access.
     * This is a basic sanity check for users, blacklisting some clearly bad
     * paths. Paths may still be problematic (e.g. indirectly leading to
     * /dev/mem) even if this returns false, it is up to the user to validate
     * that supplied paths are valid.
...
{
    // name_ is the name of the file in the directory, not including the
    // directory path itself For example, if we have directory a/b containing
    // file c, name_ will be c
    std::string name_;

    // Note that if the file represented by name_ is a symlink, type_ will be
    // the file type of the target. For example, if name_ is a symlink to a
    // directory, its file type will be Directory.
    FileType type_;
 ...
 * TODO(sesmith177): replace with std::filesystem::directory_iterator once we
 * move to C++17
 */
