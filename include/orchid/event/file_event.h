...
public:
    ...
    /**
     * Activate the file event explicitly for a set of events. Should be a
     * logical OR of FileReadyType events. This method "injects" the event (and
     * fires callbacks) regardless of whether the event is actually ready on the
     * underlying file.
     */
    virtual void activate(uint32_t events) PURE;

    /**
     * Enable the file event explicitly for a set of events. Should be a logical
     * OR of FileReadyType events. As opposed to activate(), this routine causes
     * the file event to listen for the registered events and fire callbacks
